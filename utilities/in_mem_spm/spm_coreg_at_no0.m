function x = spm_coreg_at_no0(varargin)
...
if nargin < 1
    ...
    VG = varargin{1};
    if ischar(VG), VG = spm_vol(VG); error('unintended usage'); end
end
if nargin < 2
    VF = spm_vol(spm_select(Inf,'image','Select moved image(s)'));
else
    VF = varargin{2};
    if ischar(VF) || iscellstr(VF), VF = spm_vol(char(VF)); error('unintended usage'); end;
end

if ~isfield(VG, 'uint8') %this section is still just all in the header
...
if numel(VF)~=1
    warning('probably unintended usage')
end
...
if size(V.pinfo,2)==1 && V.pinfo(1) == 2
    ...
    mx = -Inf; mn =  Inf;
    for p=1:V.dim(3)
        img = spm_slice_vol(V.img,spm_matrix([0 0 p]),V.dim(1:2),1); %V->V.img, -AT
        %I'm pretty sure I can keep all else the same
...
% Display ortho-views
%--------------------------------------------------------------------------
warning('note that images displayed through the GUI may be the wrong images!')
spm_orthviews('Reset');
