function P3=spm_reslice_at(P2,flags)
% operates on "objects" (structures)
% this function is called by spm_coreg_at
...
if ~nargin,      error('unintended use');P = spm_select([1 Inf],'image'); end
if iscellstr(P), error('unintended use');P = char(P);    end
if ischar(P),    error('unintended use');P = spm_vol(P); end
...
if isempty(P)
    ...
    [VO2,v2]=reslice_images(P,P2,flags);
    for i=1:numel(VO2)
        P3(i).hdr=VO2(i);
        P3(i).img=v2{i};
    end
%     for i=1:numel(VO2)
%         P3(i).hdr=rmfield(P3(i).hdr,'img');
%     end
end
...
%==========================================================================
function [VO2,v2]=reslice_images(P,P2,flags)
...
%don't think the actual spm_read_vols images have been referenced yet
nread = numel(P);
if ~flags.mean
...
for i = 1:numel(P)
    ...
    if read_vol
        if ~isfinite(flags.interp)
            v = abs(kspace3d(spm_bsplinc(double(P2(i).img),[0 0 0 ; 0 0 0]'),P(1).mat\P(i).mat)); %change P(i) to be double precision floating point array
            for x3 = 1:P(1).dim(3)
                if flags.mean
                    Integral(:,:,x3) = ...
                        Integral(:,:,x3) + ...
            ...
            C = spm_bsplinc(double(P2(i).img), d); %change P(i) to be double precision floating point array
            v = zeros(P(1).dim);
            for x3 = 1:P(1).dim(3)
        ...
        if write_vol % this is all that needs to be changed now
            VO         = P(i);
...
if flags.mean
    ...
    %----------------------------------------------------------------------
    error('probably unintended usage (this functionality not added with new i/o structure)')
